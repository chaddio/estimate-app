...
class main_m extends CI_Model {
     ...
     */
    function getServiceEntsWhere($field,$param,$limit=100,$offset=0){
        ...
        $query2 = $this->db2->get($this->table,$limit,$offset);
        // return result set as an associative array
     ...
     * also can send params as strings
     */
    function getServiceEntsLike($field,$param,$limit=100,$offset=0){
        if(is_array($field) && is_array($param)){
            foreach($field as $key => $value){
                $this->db2->like($field[$key],$param[$key]);
            }
        }else{
            $this->db2->like($field,$param);
        }
        $query = $this->db2->get($this->table,$limit,$offset);
        // return result set as an associative array
        $this->searchCount = count($query->result_array());
        return $query->result_array();
    }
    // get 5 rows at a time
    function getServiceEnts($limit, $row){
        $query = $this->db2->get($this->table,$limit,$row);
        if($query->num_rows()>0){
            // return result set as an associative array
            return $query->result_array();
        }
    ...
    function getIncompletes($wpid){
        $this->db2->where('createdby', $wpid);
        $query = $this->db2->get($this->table);
        $props = $query->result_array();
        foreach($props as $key => $val){
            $this->db2->where('property_id', $val['id']);
            $query = $this->db2->get('details1');
            $dtlCnt = count($query->result_array());
            $this->db2->where('property_id', $val['id']);
            $query = $this->db2->get('details2');
    ...
    function getNumServiceEnts(){
        return $this->db2->count_all($this->table);
    }
// get total number of users
    function getNumServiceEntsWhere($field='', $value=''){
        if($field && $value){
    ...
    function setModified($id){
        ...
        $this->db2->set('editedby', $this->session->userdata('wpid'));
        $this->db2->where('id', $id);
        $this->db2->update($this->table);
    }
    
    function setLocked($id, $locked){
        $this->db2->set('locked', $locked, FALSE );
        $this->db2->where('id', $id);
        $this->db2->update($this->table);
    }
    
    
    function getServiceEnt($property,$p=1){
        switch($p){
            case 1:
                $this->results = $this->getServiceEntsWhere('id', $property);
    ...
    function addServiceEntry($p1){
        switch ($p1){
            case 1:    
                ...
                $table = $this->table;
                $this->db2->set('modified', 'NOW()', FALSE );
                $this->db2->set('created', 'NOW()', FALSE );
                $this->db2->set('locked', 1, TRUE);
                $this->db2->insert($table,$data);
                $this->session->set_userdata(array('property_id' => $this->db2->insert_id()));
  ...
  public function getCSVData($columns=TRUE){
      if($columns){
          return $this->db2->query("SELECT a.id as `Property ID`,a.name as `Property Name`,a.Contact as `Contact Name`,
          ...
          $query = $this->db2->query("SELECT a.id as `Property ID`,a.name as `Property Name`,a.Contact as `Contact Name`,
            a.phone as `Phone`,a.fax as `Fax`,a.email as `Contact Email`,a.address as `Address`,a.city as `City`,a.state as `State`,
            a.zip as `Zip`, a.hours as `Business Hours`,a.editedby as `Last Edited By`,a.modified as `Last Modified`,a.createdby as `Created By`,a.created as `Created`,
            b.units as `Total Units`,b.unitsupstairs as `Units Upstairs`,b.unitsground as `Units Ground Level`,
            b.singlefamily as `Single Family`,b.townhouse as `Townhouse`,b.apartment as `Apartment`,b.studio as `Studio`,b.senior as `Senior Housing`,
  ...
  public function getStates($select=''){
    ...
    function delete($id){
        ...
        $this->db2->where('id',$id);
        $properties = $this->db2->delete();
        $this->db2->from('details1');
        $this->db2->where('property_id',$id);
        $details1 = $this->db2->delete();
        $this->db2->from('details2');
        $this->db2->where('property_id',$id);
        $details2 = $this->db2->delete();
        if($id && $details1 & $details2){
